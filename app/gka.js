...
// let [ warn, log, err ] = [console.log, console.log, console.log];
...
let generateGka = (config, cb = function () { }) => {
  ...
  let defaultOuput = path.join(defaultSrc, './gka-items');
  ...
  let gkaConfig = {
    ...
    tpl: config.template || 'canvas',
    algorithm: config.algorithm || 'left-right',
    frameduration: config.frameduration || 0.04,
...
const main = (rendererWindow) => {
  ...
  IPCMAIN.on('open-files', (event, arg) => {
    log(`main: open-files`);
    SHELL.openItem(path.join(cache.inputFileDir, './gka-items'))
  })
}
